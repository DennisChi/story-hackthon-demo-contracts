...
contract IPAccountImpl is ERC6551, IPAccountStorage, IIPAccount {
    ...
    constructor(
        ...
        if (accessController == address(0))
            revert Errors.IPAccount__ZeroAccessController();
    ...
    function isValidSigner(
        ...
        IAccessController(ACCESS_CONTROLLER).checkPermission(
            address(this),
            signer,
            to,
            selector
    ...
    /// @param value The amount of Ether to send.
    /// @param data The data to send along with the transaction.
    function _updateStateForExecute(
        address to,
        uint256 value,
        bytes calldata data
    ) internal {
    ...
    function _isValidSigner(
        ...
        return
            isValidSigner(
                signer,
                address(uint160(uint256(extraData))),
                context
            );
    }
