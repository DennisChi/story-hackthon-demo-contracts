...
contract CoreMetadataViewModule is BaseModule, ICoreMetadataViewModule {
    ...
    function updateCoreMetadataModule() external {
        coreMetadataModule = IModuleRegistry(MODULE_REGISTRY).getModule(
    ...
    function getCoreMetadata(
        address ipId
    ) external view returns (CoreMetadata memory) {
        return
            CoreMetadata({
    ...
    function getMetadataURI(address ipId) public view returns (string memory) {
        return
            IIPAccount(payable(ipId)).getString(
                coreMetadataModule,
                "METADATA_URI"
            );
    }
    ...
    function getMetadataHash(address ipId) public view returns (bytes32) {
        return
            IIPAccount(payable(ipId)).getBytes32(
                coreMetadataModule,
                "METADATA_HASH"
            );
    }
    ...
    function getRegistrationDate(address ipId) public view returns (uint256) {
        return
            IIPAccount(payable(ipId)).getUint256(
                IP_ASSET_REGISTRY,
                "REGISTRATION_DATE"
            );
    }
    ...
    function getNftTokenURI(address ipId) public view returns (string memory) {
        string memory nftTokenURI = IIPAccount(payable(ipId)).getString(
            coreMetadataModule,
            "NFT_TOKEN_URI"
        );
        if (_isEmptyString(nftTokenURI)) {
            nftTokenURI = IIPAccount(payable(ipId)).getString(
                IP_ASSET_REGISTRY,
                "URI"
            );
        }
            ...
            string(
                abi.encodePacked(
                    ...
                    Base64.encode(
                        bytes(
                            string(
                                abi.encodePacked(baseJson, ipAttributes, "]}")
                            )
                        )
                    )
                )
    ...
    function isSupported(address ipAccount) external view returns (bool) {
        return
            !_isEmptyString(
                IIPAccount(payable(ipAccount)).getString(
                    IP_ASSET_REGISTRY,
                    "NAME"
                )
            );
