...
import {ICoreMetadataModule} from "../../core/interfaces/modules/metadata/ICoreMetadataModule.sol";
import {IGroupingModule} from "../../core/interfaces/modules/grouping/IGroupingModule.sol";
import {ILicensingModule} from "../../core/interfaces/modules/licensing/ILicensingModule.sol";
import {GroupNFT} from "../../core/GroupNFT.sol";
    ...
    UUPSUpgradeable
{
    ...
    function registerGroupAndAttachLicense(
        ...
        GROUP_NFT.safeTransferFrom(
            address(this),
            msg.sender,
            GROUP_NFT.totalSupply() - 1
        );
    }
    ...
    function collectRoyaltiesAndClaimReward(
        ...
        for (uint256 i = 0; i < currencyTokens.length; i++) {
            if (currencyTokens[i] == address(0))
                revert Errors.GroupingWorkflows__ZeroAddressParam();
            collectedRoyalties[i] = GROUPING_MODULE.collectRoyalties(
                groupId,
                currencyTokens[i],
                groupSnapshotIds
            );
            GROUPING_MODULE.claimReward(
                groupId,
                currencyTokens[i],
                memberIpIds
            );
    ...
    function _getGroupingWorkflowsStorage()
        private
        pure
        returns (GroupingWorkflowsStorage storage $)
    {
        assembly {
