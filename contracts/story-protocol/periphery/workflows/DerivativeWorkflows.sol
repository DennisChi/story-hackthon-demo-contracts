...
import {ILicensingModule} from "../../core/interfaces/modules/licensing/ILicensingModule.sol";
import {IRoyaltyModule} from "../../core/interfaces/modules/royalty/IRoyaltyModule.sol";
    ...
    ERC721Holder
{
    ...
    function setNftContractBeacon(
        address newNftContractBeacon
    ) external restricted {
        if (newNftContractBeacon == address(0))
            revert Errors.DerivativeWorkflows__ZeroAddressParam();
        ...
        returns (address ipId, uint256 tokenId)
    {
        ...
        ISPGNFT(spgNftContract).safeTransferFrom(
            address(this),
            recipient,
            tokenId,
            ""
    ...
    function _collectLicenseTokens(
        ...
        if (licenseTokenIds.length == 0)
            revert Errors.DerivativeWorkflows__EmptyLicenseTokens();
        for (uint256 i = 0; i < licenseTokenIds.length; i++) {
            address tokenOwner = ILicenseToken(licenseToken).ownerOf(
                licenseTokenIds[i]
            );

            if (tokenOwner == address(this)) continue;
            if (tokenOwner != address(msg.sender))
                revert Errors.DerivativeWorkflows__CallerAndNotTokenOwner(
                    licenseTokenIds[i],
                    msg.sender,
                    tokenOwner
                );

            ILicenseToken(licenseToken).safeTransferFrom(
                msg.sender,
                address(this),
                licenseTokenIds[i]
            );
    ...
    function _collectMintFeesAndSetApproval(
        ...
        (address royaltyPolicy, , , address mintFeeCurrencyToken) = lct
            .getRoyaltyPolicy(licenseTermsIds[0]);
        ...
        if (royaltyPolicy != address(0)) {
            ...
            if (totalMintFee != 0) {
                ...
                IERC20(mintFeeCurrencyToken).safeTransferFrom(
                    payerAddress,
                    address(this),
                    totalMintFee
                );
                ...
                IERC20(mintFeeCurrencyToken).forceApprove(
                    royaltyModule,
                    totalMintFee
                );
            }
    ...
    function _aggregateMintFees(
        ...
        for (uint256 i = 0; i < parentIpIds.length; i++) {
            (, mintFee) = ILicensingModule(licensingModule)
    ...
    function _getDerivativeWorkflowsStorage()
        private
        pure
        returns (DerivativeWorkflowsStorage storage $)
    {
        assembly {
