...
// solhint-disable-next-line max-line-length
import {IGraphAwareRoyaltyPolicy} from "../../core/interfaces/modules/royalty/policies/IGraphAwareRoyaltyPolicy.sol";
import {IIpRoyaltyVault} from "../../core/interfaces/modules/royalty/policies/IIpRoyaltyVault.sol";
    ...
    UUPSUpgradeable
{
    ...
    constructor(address royaltyModule) {
        if (royaltyModule == address(0))
            revert Errors.RoyaltyWorkflows__ZeroAddressParam();
    ...
    function transferToVaultAndSnapshotAndClaimByTokenBatch(
        ...
        for (uint256 i = 0; i < royaltyClaimDetails.length; i++) {
            IGraphAwareRoyaltyPolicy(royaltyClaimDetails[i].royaltyPolicy)
                .transferToVault({
        ...
        IIpRoyaltyVault ancestorIpRoyaltyVault = IIpRoyaltyVault(
            ROYALTY_MODULE.ipRoyaltyVaults(ancestorIpId)
        );
        ...
        amountsClaimed = ancestorIpRoyaltyVault
    ...
    function transferToVaultAndSnapshotAndClaimBySnapshotBatch(
        ...
        address[] memory currencyTokens = _getCurrencyTokenList(
            royaltyClaimDetails
        );
        ...
        for (uint256 i = 0; i < currencyTokens.length; i++) {
            ...
            returns (uint256 claimedAmount) {
                ...
                if (
                    CoreErrors.IpRoyaltyVault__NoClaimableTokens.selector !=
                    bytes4(reason)
                ) {
                    assembly {
