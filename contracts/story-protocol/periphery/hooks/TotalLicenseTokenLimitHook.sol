...
import {BaseModule} from "../../core/modules/BaseModule.sol";
import {AccessControlled} from "../../core/access/AccessControlled.sol";
import {ILicensingHook} from "../../core/interfaces/modules/licensing/ILicensingHook.sol";
import {ILicenseRegistry} from "../../core/interfaces/registries/ILicenseRegistry.sol";
import {ILicenseToken} from "../../core/interfaces/ILicenseToken.sol";
import {ILicenseTemplate} from "../../core/interfaces/modules/licensing/ILicenseTemplate.sol";

contract TotalLicenseTokenLimitHook is
    BaseModule,
    AccessControlled,
    ILicensingHook
{
    ...
    error TotalLicenseTokenLimitHook_TotalLicenseTokenLimitExceeded(
        uint256 totalSupply,
        uint256 amount,
        uint256 limit
    );
    ...
    /// @param totalSupply The total supply of the license tokens
    /// @param limit The total license token limit
    error TotalLicenseTokenLimitHook_LimitLowerThanTotalSupply(
        uint256 totalSupply,
        uint256 limit
    );
    ...
    constructor(
        ...
        if (licenseRegistry == address(0))
            revert TotalLicenseTokenLimitHook_ZeroLicenseRegistry();
        if (licenseToken == address(0))
            revert TotalLicenseTokenLimitHook_ZeroLicenseToken();
    ...
    function setTotalLicenseTokenLimit(
        ...
        bytes32 key = keccak256(
            abi.encodePacked(licensorIpId, licenseTemplate, licenseTermsId)
        );
        uint256 totalSupply = _getTotalSupply(licensorIpId);
        if (limit < totalSupply)
            revert TotalLicenseTokenLimitHook_LimitLowerThanTotalSupply(
                totalSupply,
                limit
            );
        totalLicenseTokenLimit[key] = limit;
        emit SetTotalLicenseTokenLimit(
            licensorIpId,
            licenseTemplate,
            licenseTermsId,
            limit
    ...
    function supportsInterface(
        ...
        return
            interfaceId == type(ILicensingHook).interfaceId ||
    ...
    function _checkTotalTokenLimit(
        ...
        if (limit != 0) {
            ...
            if (totalSupply + amount > limit) {
                revert TotalLicenseTokenLimitHook_TotalLicenseTokenLimitExceeded(
                    totalSupply,
                    amount,
                    limit
                );
    ...
    function _calculateFee(
        ...
        (, , uint256 mintingFee, ) = ILicenseTemplate(licenseTemplate)
    ...
    function _getTotalSupply(
        ...
        return
            LICENSE_REGISTRY.getDerivativeIpCount(licensorIpId) +
