...
import {IIPAssetRegistry} from "../../core/interfaces/registries/IIPAssetRegistry.sol";
// solhint-disable-next-line max-line-length
    ...
    ERC721Holder
{
    ...
    bytes32 private constant OrgNFTStorageLocation =
        0xa4a36278839a4db2ab2cd96ad705f696fd1f52c0a329c48dd114f7acbbc8db00;

    modifier onlyStoryNFTFactory() {
        if (msg.sender != address(STORY_NFT_FACTORY)) {
            revert OrgNFT__CallerNotStoryNFTFactory(
                msg.sender,
                STORY_NFT_FACTORY
            );
        }
    ...
    function mintRootOrgNft(
        ...
        external
        onlyStoryNFTFactory
        returns (uint256 rootOrgTokenId, address rootOrgIpId)
    {
        OrgNFTStorage storage $ = _getOrgNFTStorage();
        if ($.rootOrgIpId != address(0))
            revert OrgNFT__RootOrgNftAlreadyMinted();

        (rootOrgTokenId, rootOrgIpId) = _mintAndRegisterIp(
            recipient,
            tokenURI_
        );
    ...
    function mintOrgNft(
        ...
        external
        onlyStoryNFTFactory
        returns (uint256 orgTokenId, address orgIpId)
    {
        OrgNFTStorage storage $ = _getOrgNFTStorage();
    ...
    function _mintAndRegisterIp(
        ...
        orgIpId = IP_ASSET_REGISTRY.register(
            block.chainid,
            address(this),
            orgTokenId
        );
    ...
    {
        return
            interfaceId == type(IOrgNFT).interfaceId ||
            super.supportsInterface(interfaceId);
    }

    /// @dev Returns the storage struct of OrgNFT.
    function _getOrgNFTStorage()
        private
        pure
        returns (OrgNFTStorage storage $)
    {
        assembly {
